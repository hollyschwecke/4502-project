...
def create_visualizations(year):
    ...
    try:
        wildfire_features = pd.read_csv(f'../data/features/{year}/wildfire_features_{year}.csv')
    ...
    if not month_col and 'fire_date' in wildfire_features.columns:
        wildfire_features['month'] = pd.to_datetime(wildfire_features['fire_date']).dt.month
        month_col = 'month'
    ...
    if not importance_df.empty:
        ...
        if top_feature in wildfire_features.columns:
            plt.figure(figsize=(10, 6))
            sns.scatterplot(x=top_feature, y='fire_frp', data=wildfire_features, hue=month_col if month_col else None)
    ...
    for col, pos, title in [
        ...
        if col in wildfire_features.columns:
            sns.scatterplot(x=col, y='fire_frp', data=wildfire_features, ax=ax[pos])
